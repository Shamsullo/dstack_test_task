...
def create_cloudwatch_client(aws_access_key_id, aws_secret_access_key, aws_region):
    return boto3.client('logs', aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key, region_name=aws_region)
...
def main():
    ...
    ensure_cloudwatch_log_group_and_stream(cloudwatch_client, args.aws_cloudwatch_group, args.aws_cloudwatch_stream)
    container = run_docker_container(args.docker_image, args.bash_command)
    print(f'Container {container.id} is running.')

    try:
        stream_logs_to_cloudwatch(container, cloudwatch_client, args.aws_cloudwatch_group, args.aws_cloudwatch_stream)
    finally:
        container.stop()
        container.remove()
